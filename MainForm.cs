...
namespace Clicker
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void RunAction(object actionObj)
        {
            ...
            int repeatCount = tmpAction.Repeat;
            bool forever = false;
            if (repeatCount == 0)
                forever = true;
            while (forever || repeatCount > 0)
            {
        ...
        private void Form1_KeyPress(object sender, KeyPressEventArgs e)
        {
                ...
                || e.KeyChar.Equals('r') || e.KeyChar.Equals('t'))
            {
                ...
                int interval = (int)sec;
                string text = ct.Equals(ClickType.SendKeys) ? txbEntry.Text : string.Empty;
                ListViewItem lvi = new ListViewItem(new string[] { point, ct.ToString(), interval.ToString(), text });
                ActionEntry acion = new ActionEntry(x, y, text, interval, ct);
                lvi.Tag = acion;
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            if (runActionThread == null || !runActionThread.IsAlive)
            {
                ...
                ActionsEntry tmpAction = ParseConfig();
                this.TopMost = false;
                runActionThread = new Thread(RunAction);
                runActionThread.Start(tmpAction);
            }
        ...
        private ActionsEntry ParseConfig()
        {
            ActionsEntry tmpAction = new ActionsEntry();
            List<ActionEntry> tmpActionsEntryActions = new List<ActionEntry>();
            foreach (ListViewItem lvi in lvActions.Items)
            {
                ActionEntry tmpActionEntry = lvi.Tag as ActionEntry;
                tmpActionsEntryActions.Add(tmpActionEntry);
            }
            tmpAction.Action = tmpActionsEntryActions.ToArray();
            tmpAction.StayOnTop = cbTop.Checked;
            tmpAction.Repeat = (int)numUpDownRepeat.Value;
            return tmpAction;
        }
        ...
        private void OpenFileXml(bool runIt, string file)
        {
            ...
            using (FileStream fs = System.IO.File.Open(file, FileMode.Open))
            {
                try
                {
                    ...
                    lvActions.Items.Clear();
                    foreach (ActionEntry ae in entry.Action)
                    {
                        string point = ae.X.ToString() + "," + ae.Y.ToString();
                        string interval = (ae.Interval).ToString();
                        ListViewItem lvi = new ListViewItem(new string[] { point, ae.Type.ToString(), interval, ae.Text });
                        ActionEntry acion = new ActionEntry(ae.X, ae.Y, ae.Text, ae.Interval, ae.Type);
                        lvi.Tag = acion;
                        lvActions.Items.Add(lvi);
                    }
                    cbTop.Checked = entry.StayOnTop;
                    numUpDownRepeat.Value = entry.Repeat;
